...
trait Allocator
{
	...
	#[inline(always)]
	fn map_allocation_result(_allocation_result: Result<*mut c_void, PmdkError>, _error_message: &'static str, request: &Layout) -> Result<*mut u8, AllocErr>
	{
		Err(AllocErr::Exhausted { request: request.clone() })
	}
}
